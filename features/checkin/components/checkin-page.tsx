...
export default function CheckinClient({ initialAttendees, loading, verse, isThereProgramToday }: CheckinClientProps) {    
    ...
    const { handleCheckIn, isPending, optimisticAttendees, storedNames, selectedTab, setSelectedTab } = useCheckin({ initialAttendees });
                        ...
                        {isThereProgramToday ? (
                            <>
                                <form action={(formData) => handleCheckIn(formData)} > 
                                    <TabsCheckinComponent loading={loading} storedNames={storedNames} selectedTab={selectedTab} setSelectedTab={setSelectedTab}/>
                                </form>
                                {loading ? (
