...
export default function useRegister(initialDates: Date[]) {
  ...
  const handleOptimisticDates = (
    state: Date[],
    action: OptimisticCalendarActionProp,
  ): Date[] => {
  ...
  const [optimisticDates, addOptimisticDate] = useOptimistic(
    baseDates,
    handleOptimisticDates,
  );
  ...
  const handleSelectOrUnselectData = async (updatedDate: Date) => {
    const exists = optimisticDates.some(
      (d) =>
        toLocalMidnight(d).getTime() === toLocalMidnight(updatedDate).getTime(),
    );
